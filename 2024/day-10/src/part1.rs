        ...
        .map(|(row_num, row)| {
            row.iter()
                ...
                .map(|(col_num, _)| {
                    ...
                    find_trails(0, (row_num, col_num), &map, &mut ends_of_paths);
...
fn find_trails(
    ...
    surrounding_found
        .iter()
        .map(|&coord| find_trails(elem + 1, coord, map, set))
        .collect()
}
...
fn parse_line(_input: &str) -> IResult<&str, Vec<u32>> {
    let (remaining, (numbers, _)) = many_till(
        one_of("0123456789").map(|c| c.to_digit(10).unwrap()), // question for the reader: What would this look like if we wanted to propagate the error?
        line_ending,
    )(_input)?;
    dbg!(&numbers);
