...
use nom::{bytes::complete::tag, character::complete::alphanumeric1, sequence::delimited, IResult};
...
pub fn problem1_attempt_2() -> i64 {
    ...
    loop {
        match parse_mul(text) {
            Ok((remaining, consumed)) => match parse_mul_args(remaining) {
...
fn parse_mul_args(text: &str) -> IResult<&str, &str> {
    delimited(tag("("), alphanumeric1, tag(")"))(text)
}
