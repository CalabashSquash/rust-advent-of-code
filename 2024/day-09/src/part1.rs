...
pub fn process(_input: &str) -> miette::Result<String> {
    let (remaining, (mut disk, final_id)) = parse(_input).map_err(|e| miette!("Error parsing: {}", e))?;
    if !remaining.is_empty() {
        let (_, final_size) = parse_leftover(remaining).map_err(|e| miette!("Error parsing: {}", e))?;
    ...
    let disk_reversed: Vec<(State, Option<u64>)> = disk.clone().iter().rev().map(|&slot| {
        if let Some(s) = slot {
    ...
    let (mut next_empty,_) = &disk.iter().enumerate().find(|(_, e)| {
        e.is_none()
    }).unwrap();
    ...
    for (i, (state, id)) in disk_reversed.iter().enumerate() {
        ...
        let (new_next_empty, _) = &disk.iter().skip(next_empty + 1).enumerate().find(|(_, e)| {
            e.is_none()
        }).unwrap();
    ...
    let result: u64 = disk.iter().enumerate().map(|(i, e)| {
        if let Some(val) = e {
            return i as u64 * *val;
        }
        0
...
fn parse(_input: &str) -> IResult<&str, (Vec<Option<u64>>, u64)> {
    fold_many0(parse_pair, || (Vec::new(), 0), |(mut disk, id), (file_size, free_space)| {
