...
pub fn process(_input: &str) -> miette::Result<String> {
    ...
    let mut coords_added: HashMap<Point, bool> = HashMap::new();
    for (_, p1, p2) in distances {
        add_point_pair(false, *p1, *p2, &mut circuits);
        if !coords_added.contains_key(p1) {
            coords_added.insert(*p1, true);
        }
        if !coords_added.contains_key(p2) {
            coords_added.insert(*p2, true);
        }
        if coords_added.keys().len() == coords.len() {
            println!("DONE!!! {p1:?} {p2:?}");
            return Ok((p1.0 * p2.0).to_string());
        }
    }
    Err(miette!("DIDN'T FIND"))
}
