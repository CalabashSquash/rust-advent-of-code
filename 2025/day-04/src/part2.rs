...
pub fn process(_input: &str) -> miette::Result<String> {
    let (_, mut tileboard) = all_consuming(parse)(_input).map_err(|e| miette!("Error! {e}"))?;
    // println!("{:#?}", tileboard);
    Ok(count(&mut tileboard))
}

fn count(tileboard: &mut Vec<Vec<Tile>>) -> String {
    let mut count = 0;
    loop {
        let mut to_remove: Vec<(usize, usize)> = Vec::new();
        for (row_i, row) in tileboard.iter().enumerate() {
            for (col_i, _) in row.iter().enumerate() {
                if is_valid(&tileboard, row_i, col_i) {
                    to_remove.push((row_i, col_i));
                }
            }
        }
